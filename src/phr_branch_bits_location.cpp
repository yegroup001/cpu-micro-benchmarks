...
extern "C" {
extern gadget phr_branch_bits_location_gadgets[];
}

int main(int argc, char *argv[]) {
  int loop_count = 1000;
  // match gen_phr_branch_bits_location_test
    ...
    for (int dummy_branches = min_dummy_branches;
      ...
      if (use_jit) {
        ...
        uint8_t *first_target =
            jit_main->get_cur() + (1 << (branch_toggle + 1));
        if (branch_toggle >= 3) {
          // 6 bytes jnz
...
#endif
#ifdef HOST_AARCH64
        jit_main = new jit((void *)0x10000000L, 1024 * 1024 * 16);
        entry = (gadget)jit_main->get_cur();
        ...
        entry = phr_branch_bits_location_gadgets[gadget_index];
      }
